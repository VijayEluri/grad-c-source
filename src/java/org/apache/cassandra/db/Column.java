...
public class Column implements IColumn
{
    ...
    private static ByteArrayOutputStream BYOS = new ByteArrayOutputStream(10);
    private static DataOutputStream DOS =  new DataOutputStream(BYOS);
	...
	this.name = name;
        //[MODIFIED] I try to convert value to value-size 6000(byte) -->> 3 byte * 4 = 12 byte 
//        this.value = value;
        try {
		BYOS.reset();
	//	reserve data size by K byte unit
		DOS.writeBytes(String.valueOf( (value.length) / 1000));
	}catch (IOException e) {
		e.printStackTrace();
	}
	this.value = BYOS.toByteArray();
	if(logger_.isDebugEnabled()){
		logger_.debug("This column size is " + (value.length) / 1000 +" Kbyte");
		logger_.debug("This real column size is " + (this.value.length) +" byte");
	}
